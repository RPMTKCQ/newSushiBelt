...
public class TiledAshleyConfigurator {
    ...
    public void onLoadObject(TiledMapTileMapObject tileMapObject) {
        ...
        // tag named non-player objects for interaction detection
        String objName = tileMapObject.getName();
        if (objName != null && !objName.isBlank() && !isPlayer) {
            Interactable interactable = new Interactable();
            interactable.tag = objName;
    ...
    private BodyDef.BodyType getObjectType(TiledMapTile tile) {
        ...
        if ("Prop".equals(classType)) return BodyDef.BodyType.StaticBody;
        return BodyDef.BodyType.DynamicBody;
    }

    private void addEntityPhysic(MapObjects objects, BodyDef.BodyType bodyType,
                                 Vector2 relativeTo, Entity entity) {
        if (objects.getCount() == 0) return;
        Transform transform = Transform.MAPPER.get(entity);
        Body body = createBody(objects, transform.getPosition(), transform.getScaling(),
            bodyType, relativeTo, entity);
    ...
    private Body createBody(MapObjects mapObjects, Vector2 position, Vector2 scaling,
                            BodyDef.BodyType bodyType, Vector2 relativeTo, Object userData) {
